...
struct Primitive {
    ...
    AABB box;
    Primitive() = default;
    Primitive(Material *mt, const Vec3 &v = Vec3()) : mat(mt), moving(v.length()) {}
...
struct Sphere : public Primitive {
    ...
    AABB makeAABB() const override {
        ...
        return AABB(centerT0 - Vec3(radius), centerT0 + Vec3(radius)) +
            AABB(centerT1 - Vec3(radius), centerT1 + Vec3(radius));
    }
};
